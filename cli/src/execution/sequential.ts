...
/**
 * Truncate a task title for display
 */
function formatTaskTitle(title: string, maxLen: number): string {
...
export async function runSequential(options: ExecutionOptions): Promise<ExecutionResult> {
	...
	while (true) {
		...
		// Show next task preview if there are more tasks
		if (remaining > 1) {
			const allTasks = await taskSource.getAllTasks();
			const nextTask = allTasks.length > 1 ? allTasks[1] : null;
			if (nextTask) {
				console.log(`       ${pc.dim(`Next: ${formatTaskTitle(nextTask.title, 50)}`)}`);
			}
		}
